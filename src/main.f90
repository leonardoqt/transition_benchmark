program test
	...
	use benchmark_system,  only: assign_model, assign_psi, rho0, Uini, nT, &
	                             ...
	                             evo_npi_interp, final_rho_hop_interp, &
	                             test_U, test_T, test_E!, ZY_correct_sign_full, nstate
	use model_H,           only: sz1, sz2, sz3, szn, H1, H2, H3, Hn, H_sys_single, H_sys_rotate, H_sys_parallel, H_sys_mix
	...
	select case (imodel)
		case(0)
			call assign_model(1.0d0,-1.5d0,1.5d0,dt,H_sys_single,sz,shift)
		case(1)
	...
	call final_rho_hop_interp(Ut, Tv, rho_f_npi, pop_p_npi, nqT)
	!!call test_U()
	!
	!
	!-----------------------------------
	select case (imodel)
		case(0)
			call assign_model(1.0d0,-1.5d0,1.5d0,dt/nqT,H_sys_single,sz,shift)
		case(1)
			call assign_model(1.0d0,-1.5d0,1.5d0,dt/nqT,H_sys_rotate,sz,shift)
		case(2)
			call assign_model(1.0d0,-1.5d0,1.5d0,dt/nqT,H_sys_parallel,sz,shift)
		case(3)
			call assign_model(1.0d0,-1.5d0,1.5d0,dt/nqT,H_sys_mix,sz,shift)
	end select
	!
	call assign_psi(psi)
	!-----------------------------------
	...
	!call final_psi_hop_loc01_alter(Ut, Tv, psi_f_loc, hop_p_loc01_1, pop_p_loc01_1, 1)
	...
	write(*,'("DIAB   " '//adjustl(f_sz)//'(ES14.6,1X))') rho_diag_diab
	write(*,'("NPI    " '//adjustl(f_sz)//'(ES14.6,1X))') rho_diag_npi
	write(*,'("HST    " '//adjustl(f_sz)//'(ES14.6,1X))') rho_diag_npi*0.d0
	write(*,'("LOC    " '//adjustl(f_sz)//'(ES14.6,1X))') rho_diag_loc
	write(*,*) ''
	!
	write(*,*) 'Population from actual hopping'
	write(*,'("NPI    " '//adjustl(f_sz)//'(ES14.6,1X))') pop_p_npi
	write(*,'("HST    " '//adjustl(f_sz)//'(ES14.6,1X))') pop_p_npi*0.d0
	write(*,'("LOC01-1" '//adjustl(f_sz)//'(ES14.6,1X))') pop_p_loc01_1
	write(*,'("LOC01-2" '//adjustl(f_sz)//'(ES14.6,1X))') pop_p_npi*0.d0
	write(*,'("LOC19  " '//adjustl(f_sz)//'(ES14.6,1X))') pop_p_npi*0.d0
end program test
